...
<meta charset="utf-8" />
<meta name="keywords" content="支付宝扫码付款" />
<meta name="description" content="支付宝扫码付款" />
...
.inputsty{ font-size: 16px; width: 230px; height: 28px; }
    ...
    .alipay-logo {
        ...
        background-repeat: no-repeat;
        background-image: url(<?php echo SKL_WEBROOT_PATH; ?>images/t1hhfgxxvexxxxxxxx.png);
    }
</style>
...
<div class="mi-notice mi-notice-success mi-notice-titleonly order-timeout-notice" id="J_orderPaySuccessNotice">
    <div class="mi-notice-cnt">
        <div class="mi-notice-title">
            ...
            <h3>恭喜您支付成功，<span class="ft-orange" id="J_countDownSecond">6</span> 秒后自动返回网站。</h3>
        </div>
    </div>
</div>
...
if($post['isWriteNote'] == '1'){
   echo '<img id="qrimage" src="'.SKL_WEBROOT_PATH.'images/aliqrcode.jpg?cacheTime='.$cacheTime.'" width="230" />
   ';	
}else{  
 	      
   echo '<img id="qrimage" src="'.SKL_WEBROOT_PATH.'images/aliqrcode/'.$post['money'].'/'.$post['fileName'].'?cacheTime='.$cacheTime.'" width="230" />
   ';	
}
?>           
     ...
     <div id="loading" class="ui-loading qrcode-loading" data-role="qrPayImgLoading">加载中</div>
        ...
        <div class="qrcode-img-explain fn-clear">
            ...
            <div class="fn-left">打开手机支付宝<br>扫一扫开始付款</div>
        </div>
...
<div class="qrguide-area" id="J_qrguideArea" seed="NewQr_animationClick">
    <img id="qrguideimg" src="<?php echo SKL_WEBROOT_PATH; ?>images/t13cpgxf8mxxxxxxxx.png" class="qrguide-area-img active">
...
<div class="view-switch qrcode-show fn-left" id="J_viewSwitcher" unselectable="on" onselectstart="return false;" seed="NewQr_viewSwitch">
    ...
    <div class="switch-tip switch-qrcode-tip " id="J_tip_qr">
        ...
        <div class="switch-tip-icon-wrapper">
            ...
            <img class="switch-tip-icon-img" src="<?php echo SKL_WEBROOT_PATH; ?>images/t1hhfgxxvexxxxxxxx.png" alt="支付宝图标" width="50" height="17">
        </div>
    ...
    <div class="switch-tip switch-pc-tip fn-hide" id="J_tip_pc">
        ...
        <div class="switch-tip-icon-wrapper">
            ...
            <img class="switch-tip-icon-img" src="<?php echo SKL_WEBROOT_PATH; ?>images/t1z5xfxdxmxxxxxxxx.png" alt="手机支付宝图标" width="30" height="30">
        </div>
    ...
    <div class="login-title-area">
        <div class="login-title">
            <div class="login-title-left">
                <p class="lt-text ft-yh">请将以下收款信息复制粘贴到支付宝转账页面输入框中，然后点击下一步进行付款</p>
            </div>
...
<div class="copy">
 ...
 <div class="inputEmail"> <input onClick="select()" name="Email" class="inputsty" name="input" type="text" value="<?php echo $post['email']; ?>" readonly> </div>

 <div class="inputEmail" style="padding-top:45px;"> <input onClick="select()" style="width: 100px;" class="inputsty" name="money" type="text" value="<?php echo $post['titleShort']; ?>" readonly> </div>
 
 <div class="inputEmail" style="padding-top:48px;"> <input onClick="select()" style="width: 230px;" class="inputsty" name="text" type="text" value="" readonly> </div>
</div>
            </div>
            ...
            <div class="submitContainer fn-clear">
                <div class="left-submitContainer fn-left" style="margin-left: 163px;">
                    <a rel="noreferrer" href="https://shenghuo.alipay.com/send/payment/fill.htm?title=<?php echo $post['titleShort']; ?>" target="_blank" class="newBtn-blue newBtn-long" style="width: 230px;">
                        <span>去支付宝付款</span>
                    </a>
                </div>
            </div>
...
$(function($){
	...
	//点击切换pc
	$('.switch-tip-btn').click(function(){	
		
		var J_view_qr=$('#J_view_qr');
		var J_tip_pc=$('#J_tip_pc');	
		var J_tip_qr=$('#J_tip_qr');

		if(J_view_qr.is(":hidden")){
			J_view_qr.show(500);
			J_tip_pc.hide();	
			J_tip_qr.show();		
...
var timerQuerystatus =window.setInterval("querystatus('<?php echo $post['titleLong']; ?>')",3000);
...
$('#alertNotice').show(200,function(){
	window.setTimeout("$('#alertNotice').hide(1000*3)",1000*20);
});
