...
public function _dopay(){   
  ...
  $poMoney=(float)$poMoney;
	if($this->cfg_isWriteNoteAli == '1' || $this->cfg_isWriteNoteWx == '1' || $payType == $this->tenAlias){
		 $isWriteNote=1;
	}else{
  ...
  if(empty($orderFileName['findOrder'])){
	  ...
	  if($payType == $this->aliAlias || $payType == $this->wxAlias){
      ...
      $this->cfg_moneyField=>$poMoney,//升级修改
		);
...
public function _addSklFiled(){
  ...
  /*$createSkl=$this->db->query("CREATE TABLE IF NOT EXISTS `$table` (
  ...
  `state` tinyint(2) NOT NULL DEFAULT '0' COMMENT '".$this->_charset('状态')."'
	...
	//升级修改，添加金额字段
  $chaMoney=$this->db->find('INFORMATION_SCHEMA.Columns','column_name',"table_name='$table' AND table_schema='$this->cfg_DB_NAME' AND column_name='$this->cfg_moneyField'",null,false);

	if($chaMoney['column_name'] != $this->cfg_moneyField){
	  $moneyIsOk=$this->db->query("ALTER TABLE `$table` ADD `$this->cfg_moneyField` float( 10,2 ) DEFAULT '0.00' COMMENT '".$this->_charset('金额')."'"); 

	  if(!$moneyIsOk){ skl_error('金额字段添加失败！'); }
...
public function _randOrder($isWriteNote=0,$poMoney=null,$poTitle=null,$webPayType=null,$uid=null){
  ...
  if($isWriteNote == 1){
    ...
    if($webPayType == $this->cfg_aliPayValue){
			...
			$payName='支付宝';    $qrcodeName='ali';

		}elseif($webPayType == $this->cfg_wxPayValue){
		  
			$payName='微信';		 $qrcodeName='wx';
...
$payTime+=20;//付款时间多加20秒防止服务器时间不准造成错误
...
//升级修改,开始
$addMoney=$post['money']*100;
$isAddOk=$this->db->update('user_zhanghu',"`uqianchong`=`uqianchong`+$addMoney","`userid`='".$getOrderInfo[$this->cfg_uidField]."'");
if($isAddOk < 1){  exit('upMoneyError');  }
//升级修改,结束
...
curl_setopt($ch, CURLOPT_URL, "http://www.xxx.com/");//设置你需要抓取的URL
...
public function  _fileJson($arr=array()){
  ...
  if(empty($dataArr)){
	...
	if(!is_writable($filename) || !$isWrite) {
...
$shoukuanla->publicPost=array('payType'=>$payType,'seller_email'=>$seller_email,'out_trade_no'=>$data['num'],'price'=>$cprice,'skl_money_type'=>$_REQUEST['skl_money_type'],'isMobile'=>'0');
